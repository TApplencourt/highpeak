...
int main(int argc, char **argv) {
  ...
  std::vector<float> A(global_range);
    ...
    p.build_with_source(R"EOL(__kernel void hello_world(__global float *ptr) { 
        float y = (float) get_local_id(0);
        float x = 1.3f;
        for(int j=0; j<128; j++) {
            )EOL" STRINGIFY(MAD_64(x,y)) R"EOL(;
        }
    ...
    myQueue.submit([&](sycl::handler &cgh) {
      ...
      cgh.parallel_for(sycl::nd_range<1>(global_range,128), p.get_kernel("hello_world"));
